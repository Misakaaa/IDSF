...
def prepare_dataset(path, built_vocab=None, user_only=False):
    ...
    for d in data:
        ...
        if len(dd) == 1:
            ...
            # p_data.append([t, user, tag, intent])
            history.append(user)
            T.append(dd)
...
def pad_to_batch(batch, w_to_ix, s_to_ix):  # for bAbI dataset
    ...
    for i in range(len(batch)):
        ...
        if current[i].size(1) < max_current:
            currents.append(torch.cat(
                [current[i], torch.LongTensor([w_to_ix['<pad>']] * (max_current - current[i].size(1))).view(1, -1)], 1))
        else:
            currents.append(current[i])

        if slot[i].size(1) < max_slot:
            slots.append(
                torch.cat([slot[i], torch.LongTensor([s_to_ix['<pad>']] * (max_slot - slot[i].size(1))).view(1, -1)],
                          1))
        else:
            slots.append(slot[i])
...
# train_data, word2index, slot2index, intent2index = prepare_dataset('data/train-intent1')
